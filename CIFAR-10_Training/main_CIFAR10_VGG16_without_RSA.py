...
import os
import argparse
import model_CIFAR10_VGG16_without_RSA
import data
...
def get_parser():
    ...
    parser.add_argument('--dropoutprob', type=float, default=0.5)
...
def main():
    ...
    if args.model_type == "student":
        ...
        if args.load_teacher_from_checkpoint:
            teacher_model = model_CIFAR10_VGG16_without_RSA.BigModel(args, "teacher")
        ...
        student_model = model_CIFAR10_VGG16_without_RSA.SmallModel(args, "student")
        #student_model.start_session()
        student_model.train(dataset, train_x, train_y, test_x, test_y, teacher_model)

        # Testing student model on the best model based on validation set
        student_model.start_session()
        student_model.load_model_from_file(args.checkpoint_dir)
        student_model.run_inference(test_x, test_y)

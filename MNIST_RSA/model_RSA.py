...
class SmallModel:
    ...
    def train(self, dataset, teacher_model=None):
        ...
        with tf.Session() as self.sess:
            ...
            max_accuracy = 0
            ckpt = tf.train.get_checkpoint_state('./studentcpt_GD/')
            print("Reading model parameters from %s" % ckpt.model_checkpoint_path)
            self.saver.restore(self.sess, ckpt.model_checkpoint_path)
            ...
            #run_inference_w_variation(dataset,stddevVar=0.0)
            run_inference(dataset)            
            acc_list = []
            var = tf.trainable_variables()
            print(var)
            masks = _create_mask(5)
            ...
            self._train_step = self.optimizer.apply_gradients(grads_and_vars = grads_and_vars)
            ...
            for step in range(1, self.num_steps + 1):
                ...
                _, summary = self.sess.run([self._train_step, self.merged_summary_op],
            ...
            #np.savetxt('MNIST_student_with_KD_acc.csv', acc_list,delimiter=",")
